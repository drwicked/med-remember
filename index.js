...
let subscription = button.on('detected', () => {
  ...
  const currentPresses = buttonPresses.update(today, (val) => parseInt(val) + 1, (data) => console.log(data));
...
setInterval(() => {
  ...
  const today = moment().format('YYYYMMDD')
  fb.font("fantasy", 44, true);
  fb.text(xMax - 36, yMax - 20, (buttonPresses.get(today) || 0), false, 0, true);
